    ...
    <div class="container py-12">
        <section class="grid lg:grid-cols-6 gap-6 text-white">
            <a href="{{route('orders.index'). "?status=1"}}" class="bg-red-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $pending }}</p>
                <p class="uppercase text-center">Pendiente</p>
                <p class="text-center text-2xl mt-2">
                    <i class="fas fa-business-time"></i>
                </p>
            </a >
            <a href="{{route('orders.index'). "?status=6"}}" class="bg-blue-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $reviewing }}</p>
                <p class="uppercase text-center">En Revisión</p>
                <p class="text-center text-2xl mt-2">
                    <i class="fa-solid fa-list-check text-lg"></i>
                </p>
            </a >
            <a href="{{route('orders.index'). "?status=2"}}" class="bg-gray-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $received }}</p>
            ...
            <a href="{{route('orders.index'). "?status=3"}}" class="bg-yellow-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $shipped }}</p>
            ...
            <a href="{{route('orders.index'). "?status=4"}}" class="bg-pink-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $delivered }}</p>
            ...
            <a href="{{route('orders.index'). "?status=5"}}" class="bg-green-500 bg-opacity-75 rounded-lg px-12 pt-8 pb-4">
                <p class="text-center text-2xl">{{ $nulled }}</p>
        ...
        <section class="bg-white shadow-lg rounded-lg px-12 py-8 mt-12 text-gray-700">
            ...
            <ul>
                @forelse ($orders as $order)
                                    ...
                                    <span class="w-12 text-center">
                                        @switch($order->status)
                                            ...
                                            @case(5)
                                                ...
                                                @break
                                            @case(6)
                                                <i class="fa-solid fa-list-check text-lg text-blue-500 opacity-50"></i>     
                                                @break    
                                            @default
                    ...
                    @endif
                @empty
                    <p class="text-center text-2xl">No tienes ninguna orden aún 😢</p>
                @endforelse
            </ul>
        </section>
